...
for nome_cenario, crescimento in cenarios.items():
    ...
    df[nome_cenario] = valores

# Tabela formatada
styled_df = df.copy()
styled_df["Projetos"] = projetos_coluna
for nome_cenario in cenarios:
    styled_df[nome_cenario] = styled_df[nome_cenario].apply(formatar_valor)
...
for nome_cenario in cenarios:
    ...
    y_values = df[nome_cenario.replace(".", "")]  # raw data
