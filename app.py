...
import pandas as pd
from babel.numbers import format_currency  # ✅ NOVO
...
if st.sidebar.button("Simular Orçamento"):
    ...
    # ✅ Formata os valores da coluna 'Valor Estimado' se ela existir
    if "Valor Estimado" in df.columns:
        df["Valor Estimado (R$)"] = df["Valor Estimado"].apply(lambda x: format_currency(x, 'BRL', locale='pt_BR'))
